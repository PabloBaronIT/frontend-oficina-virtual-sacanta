<template>
  <div class="card text-bg-light mb-3 container" style="min-width: 20rem">
    <div class="card-header top" @click="verTramite(obj.id)">
      ...
      <br />
      <span>{{ obj.procedure }}</span>
      <br />
    </div>
    <div class="card-body medio">
      <div>
        <h6 class="card-title">
          Asunto: <strong> {{ obj.categoria }}</strong>
        </h6>
        <p class="card-text">CUIL: {{ obj.cuil.cuil }}</p>
      </div>
    ...
    <div :style="`background:${obj.color}`" class="footer">
      <p>{{ obj.estado }}</p>
    </div>
  </div>
</template>
